...
- Изменяет статус на "Передан на БВ"

**Менеджер (опционально):**
- Видит контакт со статусом "Передан на БВ"
...
2. **Проверка истории:**
   ...
   - В секции "История статусов" должны быть все изменения:
     ...
     - Завершение (Назначен исполнитель → Передан на БВ)
...
3. **Фильтрация по району:**
   - Примените фильтр "Район: Центральный"
   - Должны отобразиться контакты из этого района
